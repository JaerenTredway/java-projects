...
public class HangmanTester {
    ...
    private static void testIsIncomplete() {
        ...
        for(String str : incompleteStrs) {
            ...
            countTest(Hangman.isIncomplete(arr));
    ...
    private static void testUpdateWithGuess() {
        ...
        countTest(!Hangman.updateWithGuess(known, word, 'e'));
        countTest(Arrays.equals("******".toCharArray(), known));
        // When the letter is present, updateWithGuess() should return true
        countTest(Hangman.updateWithGuess(known, word, 'o'));
        // updateWithGuess() should also modify the char[] that was passed in
        countTest(Arrays.equals("*oo***".toCharArray(), known));
        countTest(!Hangman.updateWithGuess(known, word, 't'));
        countTest(Arrays.equals("*oo***".toCharArray(), known));
        countTest(Hangman.updateWithGuess(known, word, 'f'));
        countTest(Arrays.equals("foo***".toCharArray(), known));
        countTest(!Hangman.updateWithGuess(known, word, 'z'));
        countTest(Arrays.equals("foo***".toCharArray(), known));
        countTest(Hangman.updateWithGuess(known, word, 'r'));
        countTest(Arrays.equals("foo**r".toCharArray(), known));

        word = "banana";
        known = "******".toCharArray();
        countTest(Hangman.updateWithGuess(known, word, 'a'));
        countTest(Arrays.equals("*a*a*a".toCharArray(), known));
        countTest(!Hangman.updateWithGuess(known, word, 'e'));
        countTest(Arrays.equals("*a*a*a".toCharArray(), known));
        countTest(Hangman.updateWithGuess(known, word, 'b'));
        countTest(Arrays.equals("ba*a*a".toCharArray(), known));
        countTest(Hangman.updateWithGuess(known, word, 'n'));
    ...
    private static void testPickRandomWord() {
        ...
        for (int i = 0; i < n; i++) {
            String word = Hangman.chooseRandomWord(dictionary);
